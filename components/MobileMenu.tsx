                    ...
                    return (
                      ...
                      >
                        <Link
                          href={link.href}
                          onClick={() => setIsOpen(false)}
                          className={`flex items-center justify-between rounded-xl p-4 text-sm font-bold tracking-widest uppercase transition-all ${isActive ? "bg-white/5 text-white" : "text-white/50 hover:bg-white/5 hover:text-white"
                            }`}
                        >
                          {link.name}
                          <div className={`h-1.5 w-1.5 rounded-full bg-primary transition-all ${isActive ? "opacity-100" : "opacity-0"
                            }`} />
                        </Link>
                      </motion.div>
                    );
