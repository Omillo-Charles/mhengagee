...
export default function FeatureRequest() {
	...
	return (
		<section aria-labelledby="feature-request-heading" className="mt-16 overflow-hidden rounded-[2px] bg-navy text-white sm:mt-20">
			<div className="grid lg:grid-cols-[0.85fr_1.15fr]">
				<div className="flex flex-col justify-between bg-primary p-6 sm:p-10 lg:p-14">
					<div>
						<p className="font-accent text-[10px] font-semibold uppercase tracking-[0.2em] text-white/65">Be part of the conversation</p>
						<h2 id="feature-request-heading" className="mt-3 max-w-md font-display text-4xl font-bold leading-[0.94] tracking-[-0.035em] sm:text-5xl">Have a story worth hearing?</h2>
						<p className="mt-5 max-w-md text-sm leading-7 text-white/75">Request to be featured on the next podcast and tell us what you are building, learning, or thinking about.</p>
					</div>
				...
				<div className="p-6 sm:p-10 lg:p-14">
					{submitted ? (
						<div className="flex min-h-[420px] flex-col justify-center">
							...
							<p className="mt-3 max-w-md text-sm leading-6 text-white/65">Thanks for putting your story forward. This form is ready to connect to the podcast inbox when the backend is wired up.</p>
							<button type="button" onClick={() => setSubmitted(false)} className="mt-7 w-fit font-accent text-[10px] font-semibold uppercase tracking-[0.16em] text-accent-cyan transition-colors hover:text-white">Send another request</button>
						</div>
					) : (
						<form onSubmit={handleSubmit} className="space-y-6">
							<div className="border-b border-white/15 pb-5"><p className="font-accent text-[10px] font-semibold uppercase tracking-[0.2em] text-accent-cyan">Guest request</p><h3 className="mt-2 font-display text-3xl font-bold">Tell us about you.</h3></div>
							<div className="grid gap-6 sm:grid-cols-2">
								<label className="block"><span className="font-accent text-[10px] font-semibold uppercase tracking-[0.16em] text-white/55">Your name</span><input required name="name" type="text" placeholder="Your name" className="mt-2 w-full border-b border-white/20 bg-transparent px-0 py-3 text-sm text-white outline-none placeholder:text-white/30 focus:border-accent-cyan" /></label>
								<label className="block"><span className="font-accent text-[10px] font-semibold uppercase tracking-[0.16em] text-white/55">Email address</span><input required name="email" type="email" placeholder="you@example.com" className="mt-2 w-full border-b border-white/20 bg-transparent px-0 py-3 text-sm text-white outline-none placeholder:text-white/30 focus:border-accent-cyan" /></label>
							</div>
