	...
	return (
		...
		>
			<div className={`relative overflow-hidden bg-navy ${featured ? "aspect-[16/8]" : "aspect-[16/10]"}`}>
				<img src={story.coverImage} alt={story.title} className="h-full w-full object-cover object-top transition duration-700 group-hover:scale-105" />
			...
			<div className="p-5 sm:p-6">
				<div className="flex items-center gap-2 font-accent text-[10px] uppercase tracking-[0.12em] text-black/40">
					<span>{story.publishedAt ? new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", year: "numeric" }).format(new Date(story.publishedAt)) : "Recently published"}</span>
				...
				<p className="mt-3 line-clamp-2 font-sans text-sm leading-6 text-black/55">{story.description}</p>
