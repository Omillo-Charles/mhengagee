...
function NewsSkeleton() {
  return (
    <div className="grid animate-pulse gap-5 md:grid-cols-2 lg:gap-6">
      <div className="h-[360px] rounded-[2px] bg-black/10 md:col-span-2" />
      {[1, 2, 3, 4].map((item) => (
        <div
          key={item}
        ...
        >
          ...
          <div className="h-7 w-4/5 rounded bg-black/10" />
...
export default function NewsResults({ category }: NewsResultsProps) {
  ...
  useEffect(() => {
    ...
    newsApi
      .list({ category })
      .then(({ data }) => {
        if (isCurrent) setStories(Array.isArray(data) ? data : []);
      })
      .catch(() => {
        if (isCurrent)
          setErrorMessage(
    ...
    return (
      <div className="border border-dashed border-black/15 bg-white p-10">
        ...
        <p className="mt-3 text-sm leading-6 text-black/55">
          {category
            ? `There are no published stories in ${category} yet.`
