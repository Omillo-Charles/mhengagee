    ...
    <aside className="flex flex-col justify-between border-b border-black/10 bg-[#f8f7f3] p-5 lg:border-b-0 lg:border-r">
      <div>
        <div className="mb-8">
          ...
          <h2 className="mt-2 font-display text-2xl font-bold tracking-[-0.03em] text-navy">
            Mhengagee
          </h2>
        </div>
        ...
        <nav className="space-y-2">
          {navItems.map(({ id, label, icon: Icon }) => (
            <button
              ...
              className={`flex w-full items-center justify-between gap-3 rounded-[2px] px-3 py-3 text-left font-accent text-[10px] font-semibold uppercase tracking-[0.14em] transition-colors ${
                activeSection === id
                  ? "bg-navy text-white"
