...
function EditorHeader({ label, onClose, isSaving }: Pick<EditorProps, "label" | "onClose" | "isSaving">) {
    return (
        <div className="flex items-center justify-between gap-4 border-b border-black/10 pb-4">
            <h3 className="font-display text-2xl font-bold text-navy">{label}</h3>
            <button type="button" onClick={onClose} disabled={isSaving} className="rounded-full border border-black/10 p-2 text-navy hover:text-primary disabled:cursor-not-allowed disabled:opacity-50">
                <X size={14} />
            </button>
        </div>
    );
...
function EditorActions({ onClose, isSaving }: { onClose: () => void; isSaving: boolean }) {
    return (
        <div className="flex items-center justify-end gap-3 border-t border-black/10 pt-4">
            <button type="button" onClick={onClose} disabled={isSaving} className="font-accent text-[10px] font-semibold uppercase tracking-[0.14em] text-black/55 hover:text-navy disabled:cursor-not-allowed disabled:opacity-50">Cancel</button>
            <button type="submit" disabled={isSaving} className="inline-flex items-center gap-2 bg-navy px-4 py-2.5 font-accent text-[10px] font-semibold uppercase tracking-[0.16em] text-white transition-colors hover:bg-primary disabled:cursor-wait disabled:opacity-60" aria-busy={isSaving}>
                {isSaving ? <LoaderCircle size={14} className="animate-spin" /> : <Check size={14} />}
...
export function AdminEditors({ editor, label, isSaving, onClose, onPortfolioSubmit }: EditorProps) {
    return <PortfolioEditor item={editor.item} label={label} isSaving={isSaving} onClose={onClose} onSubmit={onPortfolioSubmit} />;
}

function PortfolioEditor({ item, label, isSaving, onClose, onSubmit }: { item?: PortfolioItem; label: string; isSaving: boolean; onClose: () => void; onSubmit: EditorProps["onPortfolioSubmit"] }) {
    return (
