...
function PodcastSkeleton() {
  return (
    <div className="grid animate-pulse gap-6 sm:grid-cols-2 lg:grid-cols-3">
      <div className="h-[440px] rounded-[2px] bg-black/10 sm:col-span-2 lg:col-span-3" />
      {[1, 2, 3].map((item) => (
        <div
          key={item}
          className="space-y-4 border border-black/5 bg-white p-4"
        >
          <div className="aspect-[16/10] rounded-[2px] bg-black/10" />
...
export default function PodcastsPage() {
  ...
  useEffect(() => {
    podcastApi
      .youtube({ limit: 12 })
      .then(({ data, nextPageToken: token }) => {
        setEpisodes(data);
        setNextPageToken(token);
      })
      .catch(() =>
        setErrorMessage(
          "Podcasts are unavailable right now. Please check back shortly.",
        ),
      )
  ...
  const loadMoreEpisodes = async () => {
    ...
    try {
      const response = await podcastApi.youtube({
        limit: 12,
      ...
      setErrorMessage(
        "More podcasts could not be loaded right now. Please try again.",
      );
    } finally {
    ...
    <main className="min-h-screen bg-[#f5f4f0] selection:bg-primary selection:text-white">
      <section className="mx-auto max-w-[1440px] px-5 pb-20 pt-8 sm:px-8 sm:pt-12 lg:px-12 lg:pt-16">
        ...
        {isLoading ? (
          ...
          <div className="border border-dashed border-black/15 bg-white p-10">
            <h1 className="font-display text-3xl font-bold text-navy">
              No podcast videos published yet.
            </h1>
            <p className="mt-3 text-sm leading-6 text-black/55">
              New conversations will appear here when they are published on
              YouTube.
            </p>
          </div>
        ) : (
          <>
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {otherEpisodes.map((episode) => (
                <article
                  key={episode.id}
                  className="group overflow-hidden rounded-[2px] border border-black/10 bg-white transition-shadow duration-300 hover:shadow-[0_18px_45px_rgba(15,23,42,0.12)]"
                >
                  <a
                  ...
                  >
                    ...
                    <span className="absolute bottom-4 right-4 flex h-10 w-10 items-center justify-center rounded-full bg-[#FF0000] text-white">
