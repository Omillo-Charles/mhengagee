...
export default function AdminPage() {
            ...
            try {
                ...
                setPortfolio(
                    portfolioResponse.data.map((item, index) => ({
                        id: item.id,
                        title: item.isFeatured ? "Featured portfolio image" : `Portfolio image ${index + 1}`,
                ...
                setLoadError("The dashboard could not load your stored content. Please try again in a moment.");
            } finally {
                setIsLoading(false);
            }
        }
    ...
    const overviewStats = useMemo(() => [
        { label: "Portfolio items", value: String(portfolio.length).padStart(2, "0"), tone: "text-navy" },
        { label: "YouTube podcasts", value: String(podcastCount).padStart(2, "0"), tone: "text-primary" },
    ...
    const handlePortfolioSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = editor?.kind === "portfolio" && editor.mode === "edit" && editor.item
                ? await portfolioApi.update(editor.item.id, formData)
                : await portfolioApi.create(formData);
            const item = response.data;
            const mappedItem: PortfolioItem = {
                id: item.id,
                title: editor?.kind === "portfolio" && editor.item ? editor.item.title : file.name.replace(/\.[^/.]+$/, ""),
                category: editor?.kind === "portfolio" && editor.item ? editor.item.category : "Photography",
                image: item.image,
                width: editor?.kind === "portfolio" && editor.item ? editor.item.width : 1200,
                height: editor?.kind === "portfolio" && editor.item ? editor.item.height : 1600,
                feature: item.isFeatured,
                publishedAt: item.publishedAt,
            };

            if (editor?.kind === "portfolio" && editor.mode === "edit" && editor.item) {
                setPortfolio((items) => items.map((existing) => (existing.id === item.id ? mappedItem : existing)));
            } else {
                setPortfolio((items) => [mappedItem, ...items]);
            }
            closeEditor();
        } catch (error) {
            setLoadError(error instanceof Error ? error.message : "The portfolio image could not be saved.");
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const handleDelete = async (kind: EditorState["kind"], id: string) => {
        ...
        try {
            ...
            setLoadError(error instanceof Error ? error.message : "The portfolio image could not be deleted.");
        }
    };
    ...
    const renderPortfolioPanel = () => (
        <div className="space-y-6">
            ...
            {editor?.kind === "portfolio" && <AdminEditors editor={editor} label={activeEditorLabel} isSaving={isSaving} onClose={closeEditor} onPortfolioSubmit={handlePortfolioSubmit} />}
        ...
        <div className="space-y-6">
            <div className="flex flex-col gap-4 border-b border-black/10 pb-5 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex gap-3">
                    <button type="button" onClick={() => { setActiveSection("portfolio"); openAdd("portfolio"); }} className="inline-flex items-center gap-2 bg-navy px-4 py-2.5 font-accent text-[10px] font-semibold uppercase tracking-[0.16em] text-white transition-colors hover:bg-primary">
                        <Plus size={14} />
                        Add portfolio
                    </button>
                </div>
                ...
                <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
                    <div className="rounded-[2px] border border-black/10 bg-white p-5">
                        <div className="mb-5 flex items-center justify-between">
                            <h2 className="font-display text-2xl font-bold text-navy">Quick summary</h2>
                            <span className="font-accent text-[10px] font-semibold uppercase tracking-[0.14em] text-black/40">Live</span>
                        </div>

                        <div className="space-y-4">
                            {[
                                {
                                    title: latestPortfolioEntry ? latestPortfolioEntry.title : "No portfolio item published yet",
                                    type: "Portfolio",
                                    time: latestPortfolioEntry?.publishedAt ? new Date(latestPortfolioEntry.publishedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : "Awaiting content",
                                },
                                { title: `${podcastCount} videos from YouTube`, type: "Podcasts", time: "Live feed" },
                            ].map((item) => (
                                <div key={`${item.type}-${item.title}`} className="flex items-center justify-between gap-4 rounded-[2px] border border-black/10 bg-[#f8f7f3] p-4">
                                    <div>
                    ...
                    <div className="rounded-[2px] border border-black/10 bg-navy p-5 text-white">
                        <p className="font-accent text-[10px] font-semibold uppercase tracking-[0.18em] text-accent-cyan">Checklist</p>
                        <h2 className="mt-3 font-display text-2xl font-bold">Before publishing</h2>
